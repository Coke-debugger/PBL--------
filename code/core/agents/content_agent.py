...
class ContentAgent(BaseAgent):
    ...
    def _build_annotation_prompt(self, lesson_text, profile, experiences):
        ...
        return (
            ...
            f"【教案】\n{lesson_text}\n\n"
            "找出知识性错误（公式/方程式/计算/事实），"
            "★只报最严重的 1~2 个错误（重大知识性错误优先，如方程式配平错、定律表述错、例题答案错），"
            "不要贪多列所有错误——输出过长会被截断导致整体解析失败，报准最重的 1~2 个即可。\n"
            "输出JSON数组（1~2条），每条含issue_id/dimension/severity/location/quote/problem/suggestion/also_replace/in_scope。\n"
            ...
            "★also_replace 字段（极重要，防C矛盾）：若该错误结论/错误数据在教案中【其他位置也出现】（哪怕措辞略不同），"
            "把这些旧表述逐字列入 also_replace 数组（每条是该旧表述的原文片段，≤30字）。程序会把 quote 和 also_replace "
            "里的每一处都替换成 suggestion，保证全文同一结论一致，避免'改了一处、别处还矛盾'导致C维度扣分。"
            "若错误只在 quote 一处出现，also_replace 填空数组 []。\n"
